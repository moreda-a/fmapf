...
public class PRIMA_Game extends Game {
	...
	public PRIMA_Board firstBoard;
	public PRIMA_State firstState;

	public PRIMA_Game(boolean centralized, String testCase, String testCase2, int num) {
	...
	private void getInfoFromInput() {
		...
		try {
			...
			firstBoard = new PRIMA_Board(this);
			firstState = new PRIMA_State(this, firstBoard);
			Game.endTime = 30;
			...
			for (int i = 1; i <= playerNumber; ++i) {
				...
				firstState.lastMove[i] = PRIMA_State.pos[x][y];
				y = sc2.nextInt();
				x = sc2.nextInt();
				firstBoard.table[x][y] = -i;
				firstState.target[i] = PRIMA_State.pos[x][y];
				sc2.next();
			}
	...
	private void setFirstState() {
		...
		gameState = new PRIMA_State(firstState);
	}
